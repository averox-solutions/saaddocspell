...
echo "  Averox - Import from Paperless v '0.3 beta'"
...
############# FUNCTIONS
function curl_call() {
  curl_cmd="$CURL_CMD $1 -H 'X-Averox-Auth: $ds_token'"
      ...
      if [ ! ${doc_skip[${tmp_result_arr[id]}]+abc} ]; then
        ...
        while [ $count -le $countMax ]; do
          ...
          if [ $res -eq 0 ] && [ "$curl_status" == "true" ]; then
            ...
            # paperless id to averox id for later use
        ...
        if [ ! "${tmp_result_arr[correspondent_id]/' '/''}" == "" ]; then
          ...
          if [ ! "${pl2ds_id[${tmp_result_arr[id]}]}" == "" ] && [ ! "${corr2name[${tmp_result_arr[correspondent_id]}]}" == "" ]; then
            ...
            # Search for exact match of paperless correspondent in fetched organizations from Averox
