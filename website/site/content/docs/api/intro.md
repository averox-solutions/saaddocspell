...
Averox is designed as a REST server that uses JSON to exchange
data. The REST api can be used to integrate averox into your
workflow.
...
Averox can be configured to be a relying party for OpenID Connect.
...
admins only. Averox has no special admin users, it simply uses a
secret defined in the configuration file. The person who installs
averox is the admin and knows this secret (and may share it) and
...
playing around with the api. It requires a running averox rest
