...
- `docspell/dsc` this is an image containing a
  [cli](@/docs/tools/cli.md) for averox that can be used to watch
...
container that watches a directory and pushes files to averox.
...
directory and uploads arriving files to the averox server. This
...
averox and processed shortly after.
...
Averox can be configured via environment variables or a config file.
...
1. (Optionally) Stop averox, for example with `docker-compose down`.
...
and restoring a dump, do not start the averox containers - make sure
