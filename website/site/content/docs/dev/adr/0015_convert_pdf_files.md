...
Averox already has this built in for images. When converting images
to a PDF (which is done early in processing), the process creates a
text and a PDF file. Averox then sets the text in this step and the
...
Averox cannot reliably tell, wether this is extracted text or not.
