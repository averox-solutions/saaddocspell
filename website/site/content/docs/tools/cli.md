...
averox through the command line. It is also aims to be useful for
...
example the averox url, the admin secret etc. They can be overriden
...
The `upload` subcommand can upload files to averox. This is the
...
everything out of averox in some independent format. Files are
...
is detected, it is pushed to averox.
...
the `consumedir` container will push it to averox.
