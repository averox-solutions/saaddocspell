...
val joex = project
  ...
  .settings(
    name := "averox-joex",
    description := "The joex component (job executor) for averox which executes long-running tasks.",
    packageSummary := "Averox",
...
val restserver = project
  ...
  .settings(sharedSettings)
  .withTestSettings
  .settings(debianSettings("averox-server"))
  .settings(buildInfoSettings)
  .settings(
    name := "averox-restserver",
    description := "Averox server providing the user interface and a REST Api.",
    packageSummary := "Averox Rest server",
    packageDescription := description.value,
    libraryDependencies ++=
      Dependencies.http4sServer ++
        Dependencies.http4sCirce ++
